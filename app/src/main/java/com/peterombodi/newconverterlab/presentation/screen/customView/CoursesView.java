...
public class CoursesView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		drawText(canvas, subTitleSize, subTitleStyle, subTitleColor, bank.getAddress(), true, LEFT_X_TITLE);
		drawText(canvas, subTitleSize, subTitleStyle, subTitleColor, getResources().getString(R.string.phone).concat(bank.getPhone()), true, LEFT_X_TITLE);
		for (Currency course : courseList) {
			...
			String resultText = staticSb
				.append((leadingZero + course.getAsk()).substring(course.getAsk().length()))
				.append(" / ")
				.append((leadingZero + course.getBid()).substring(course.getBid().length()))
				.toString();
			drawText(canvas, valueSize, valueStyle, valueColor, resultText,
				false, LEFT_X_COURSE);
		}
		currentY = 0f;
	}
	...
	private void drawText(Canvas _canvas, float _textSize, int _textStyle, int _textColor, String _text, boolean _newLine, float _x) {
		...
		_canvas.drawText(_text, _x, currentY, paintForeground);
	}
